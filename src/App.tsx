...
function App() {
  ...
  return <div className={isLoggedIn ? 'app-shell' : 'app-shell logged-out'}>
    ...
    {showRegister && <RegisterForm onClose={() => setShowRegister(false)} onDone={async (draft) => { try { const created = await saveClient(draft); window.localStorage.setItem('alugae-authenticated', 'true'); window.localStorage.setItem('alugae-role', 'customer'); window.localStorage.setItem('alugae-user-email', created.email); setCurrentUserEmail(created.email); setIsLoggedIn(true); setIsAdmin(false); setShowRegister(false); setNotice(`Cadastro de ${draft.name} criado como usuário`) } catch (error) { setNotice(error instanceof Error ? `Não foi possível cadastrar: ${error.message}` : 'Não foi possível cadastrar o cliente') } }} />}
...
function AdminView({ view, products, clients, orders, onAdd, onEdit, onAddClient, onUpdateClient, onRemove, onRemoveClient, onBack }: { view: View; products: Product[]; clients: Client[]; orders: Order[]; onAdd: () => void; onEdit: (product: Product) => void; onAddClient: () => void; onUpdateClient: (client: Client) => void; onRemove: (id: number) => void; onRemoveClient: (id: number) => void; onBack: () => void }) {
  ...
  return <main className="admin-page"><div className="admin-header"><div><button className="back-link" onClick={onBack}>← Voltar ao catálogo</button><p className="eyebrow">Painel de controle</p><h1>{title}</h1><p>{subtitle}</p></div><button className="primary-button" onClick={onAdd}><Plus size={17} /> Novo produto</button></div><div className="dashboard-cards"><div><span>Ativos</span><strong>{products.length}</strong><small className="positive">+ 12% este mês</small></div><div><span>Em andamento</span><strong>32</strong><small>atualizado agora</small></div><div><span>Receita estimada</span><strong>R$ 18.420</strong><small className="positive">+ 8,4% este mês</small></div></div><div className="data-panel"><div className="panel-head"><h2>Inventário</h2><button className="filter-button"><Filter size={16} /> Filtrar</button></div><div className="table-wrap"><table><thead><tr><th>Produto</th><th>Categoria</th><th>Valor / dia</th><th>Estoque</th><th></th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td><div className="table-product"><img src={product.image} alt="" /><strong>{product.name}</strong></div></td><td>{product.category}</td><td>{money(product.price)}</td><td><span className={product.stock < 4 ? 'stock low' : 'stock'}>{product.stock} un.</span></td><td><div className="row-actions"><button aria-label="Editar" onClick={() => onEdit(product)}><Pencil size={16} /></button><button aria-label="Excluir" onClick={() => onRemove(product.id)}><Trash2 size={16} /></button></div></td></tr>)}</tbody></table></div></div></main>
}
